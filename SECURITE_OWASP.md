...
| A07 | **Identification & Auth Failures** | ✅ / 🟡 | bcrypt + JWT (exp. 8h) + **rate-limiting** sur `/auth/login` (10 essais / 15 min). Message d'erreur générique (pas d'énumération d'utilisateurs). 🟡 Pas de MFA / politique de complexité de mot de passe (back-office interne, faible exposition). |
