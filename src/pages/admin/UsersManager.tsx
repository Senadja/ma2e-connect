...
import { Plus, Edit2, Trash2, X, ShieldCheck, UserPlus, Mail } from "lucide-react";
...
export const UsersManager = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: (f: typeof emptyForm) => {
      ...
      if (f.id) return api(`/users/${f.id}`, { method: "PUT", auth: true, body });
          ...
          {users.map((u) => (
            <div key={u.id} className="flex items-center justify-between p-4 hover:bg-secondary/10 transition-colors gap-4">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <Button variant="ghost" size="icon" className="h-9 w-9" onClick={() => setEditing({ id: u.id, email: u.email, name: u.name, password: "", role: u.role, permissions: [...u.permissions] })}>
          ...
          <Card className="w-full max-w-lg bg-card relative z-10 shadow-2xl rounded-2xl overflow-hidden animate-scale-in flex flex-col max-h-[90vh]">
            ...
            <CardContent className="p-6 overflow-y-auto space-y-5">
              ...
              <div className="grid gap-2">
                ...
                <Input type="email" value={editing.email} disabled={!!editing.id} onChange={(e) => setEditing({ ...editing, email: e.target.value })} placeholder="nom@ma2e.ci" />
              </div>
              <div className="grid gap-2">
                ...
                <div className="flex gap-2">
                  {(["EDITOR", "ADMIN"] as const).map((r) => (
                    <button key={r} type="button" onClick={() => setEditing({ ...editing, role: r })}
                      ...
                      {r === "ADMIN" ? "Administrateur" : "Gestionnaire"}
                    </button>
                  ))}
                </div>
              </div>
