  ...
  return (
    <Layout>
      <PageHero
        ...
        subtitle="Notre équipe est à votre écoute pour toute question ou demande d'information."
          ...
          <div className="grid lg:grid-cols-2 rounded-3xl overflow-hidden shadow-elegant border border-border bg-card">
            ...
            <aside className="bg-primary-dark text-white p-8 md:p-12 relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <p className="mt-2 text-white/70 text-sm">Siège social MA2E — Abidjan, Plateau</p>
            ...
            <div className="p-8 md:p-12">
              <h2 className="font-display text-3xl font-bold">Envoyez-nous un message</h2>
              <p className="mt-2 text-muted-foreground text-sm">Réponse sous 48 heures ouvrées.</p>

              {sent ? (
                <div className="mt-8 rounded-xl bg-primary/10 border border-primary/20 p-6 text-center animate-scale-in">
                  <div className="font-display text-xl font-bold text-primary">Message envoyé ✓</div>
                  <p className="text-sm text-muted-foreground mt-2">Nous vous recontactons rapidement. Merci !</p>
                </div>
              ) : (
                <form className="mt-8 space-y-4" onSubmit={(e) => { e.preventDefault(); setSent(true); }}>
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input id="nom" required className="w-full rounded-lg border border-input bg-background px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1.5" htmlFor="email">Email *</label>
                      <input id="email" type="email" required className="w-full rounded-lg border border-input bg-background px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary" />
                    </div>
                  </div>
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium mb-1.5" htmlFor="tel">Téléphone</label>
                      <input id="tel" className="w-full rounded-lg border border-input bg-background px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1.5" htmlFor="objet">Objet</label>
                      <select id="objet" className="w-full rounded-lg border border-input bg-background px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary">
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1.5" htmlFor="msg">Message *</label>
                    <textarea id="msg" rows={5} required className="w-full rounded-lg border border-input bg-background px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary" />
                  </div>
                  <label className="flex gap-2 items-start text-sm text-muted-foreground">
                    <input type="checkbox" required className="mt-1 accent-primary" />
                    <span>J'accepte que mes données soient traitées pour répondre à ma demande, conformément à la politique DCP de la MA2E. *</span>
                  </label>
                  <button type="submit" className="w-full inline-flex items-center justify-center gap-2 rounded-full bg-gradient-gold text-accent-foreground px-5 py-3.5 font-semibold shadow-gold hover:scale-[1.01] transition-bounce">
